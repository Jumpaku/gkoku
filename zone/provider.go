	...
	for _, zj := range tzotJSON {
		...
		for _, rj := range zj.Rules {
			...
			rules = append(rules, rule{
				OffsetMinutesBefore: OffsetMinutes(rj.OffsetSecondsBefore / 60),
				OffsetMinutesAfter:  OffsetMinutes(rj.OffsetSecondsAfter / 60),
				Month:               rj.Month,
				BaseDay:             rj.BaseDay,
				DayOfWeek:           rj.DayOfWeek,
				SecondOfDay:         sod,
				TimeOffsetMinutes:   OffsetMinutes(om),
			})
		}

...
func main() {
	...
	for _, year := range years {
		...
		for yd := 0; yd < date.GoDate(year, 12, 31).YearDay(); yd++ {
			sut := y0101.AddDate(0, 0, yd)
			y, m, d := sut.Date()
			sutY, sutW := sut.ISOWeek()
			sutDow := ((sut.Weekday() + 6) % 7) + 1

			fmt.Fprintf(out, "%d %d %d %d %d %d\n", sutY, sutW, sutDow, y, m, d)
		}
	}
	log.Println("Date testcases successfully generated in " + outputPath)
}

...
func TestDateIterator_Diff(t *testing.T) {
	tests := []struct {
		...
		{
			name: "zero",
			sut:  OfDate(calendar.DateOfYMD(2024, calendar.MonthMarch, 9)),
			in:   OfDate(calendar.DateOfYMD(2024, calendar.MonthMarch, 9)),
			want: 0,
		},
		{
			name: "positive",
			sut:  OfDate(calendar.DateOfYMD(2024, calendar.MonthMarch, 9)),
			in:   OfDate(calendar.DateOfYMD(2024, calendar.MonthMarch, 8)),
			want: 1,
		},
		{
			name: "negative",
			sut:  OfDate(calendar.DateOfYMD(2024, calendar.MonthMarch, 9)),
			in:   OfDate(calendar.DateOfYMD(2024, calendar.MonthMarch, 10)),
			want: -1,
		},
	}
...
func TestDateIterator_Move(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			sut:  OfDate(calendar.DateOfYMD(2024, calendar.MonthMarch, 9)),
			in:   0,
			want: OfDate(calendar.DateOfYMD(2024, calendar.MonthMarch, 9)),
		},
		{
			name: "next",
			sut:  OfDate(calendar.DateOfYMD(2024, calendar.MonthMarch, 9)),
			in:   1,
			want: OfDate(calendar.DateOfYMD(2024, calendar.MonthMarch, 10)),
		},
		{
			name: "prev",
			sut:  OfDate(calendar.DateOfYMD(2024, calendar.MonthMarch, 9)),
			in:   -1,
			want: OfDate(calendar.DateOfYMD(2024, calendar.MonthMarch, 8)),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {

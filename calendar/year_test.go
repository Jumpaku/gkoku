...
import (
	...
	test_date "github.com/Jumpaku/tokiope/calendar/internal/cmd/testcases/date"
...
func TestYear_FirstDate(t *testing.T) {
	for sut := calendar.Year(-9999); sut <= 9999; sut++ {
		t.Run(fmt.Sprintf(" %d", sut), func(t *testing.T) {
			got := sut.FirstDate()
			want := calendar.DateOfYD(int(sut), 1)
...
func TestYear_LastDate(t *testing.T) {
	for sut := calendar.Year(-9999); sut <= 9999; sut++ {
		t.Run(fmt.Sprintf(" %d", sut), func(t *testing.T) {
			got := sut.LastDate()
			want := calendar.DateOfYD(int(sut), sut.Days())
			calendar_test.AssertEqualDate(t, want, got)
		})
	}
}
	...
	for sut := calendar.Year(-9999); sut <= 9999; sut++ {
		for _, doy := range []int{1, 2, sut.Days() - 1, sut.Days()} {
			t.Run(fmt.Sprintf("%d-%d", sut, doy), func(t *testing.T) {
				got := sut.Date(doy)
				want := calendar.DateOfYD(int(sut), doy)
				calendar_test.AssertEqualDate(t, want, got)
			})
		}
	}

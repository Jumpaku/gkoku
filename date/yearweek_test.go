...
func TestYearWeek_YyyyWw(t *testing.T) {
	...
	for _, testcase := range testcases {
		t.Run(fmt.Sprintf("%d-%d", testcase.sutYear, testcase.sutWeek), func(t *testing.T) {
			sut := date.YearWeekOf(testcase.sutYear, testcase.sutWeek)
			{
...
func TestYearWeek_Date(t *testing.T) {
	for _, testcase := range testcasesYearWeek {
		sut := date.YearWeekOf(testcase.sutYear, testcase.sutWeek)
		for day := date.DayOfWeekMonday; day <= date.DayOfWeekSunday; day++ {
			t.Run(fmt.Sprintf("%d-W%d-%d", testcase.sutYear, testcase.sutWeek, int(day)), func(t *testing.T) {
				got := sut.Date(day)
				want := date.YyyyWwD(testcase.sutYear, testcase.sutWeek, day)
...
func TestYearWeek_FirstDate(t *testing.T) {
	for _, testcase := range testcasesYearWeek {
		...
		t.Run(fmt.Sprintf("%d-%d", testcase.sutYear, testcase.sutWeek), func(t *testing.T) {
			got := sut.FirstDate()
			want := date.YyyyWwD(testcase.sutYear, testcase.sutWeek, date.DayOfWeekMonday)
			calendar_test.AssertEqualDate(t, want, got)
		})
	}
}
...
func TestYearWeek_LastDate(t *testing.T) {
	for _, testcase := range testcasesYearWeek {
		...
		t.Run(fmt.Sprintf("%d-%d", testcase.sutYear, testcase.sutWeek), func(t *testing.T) {
			got := sut.LastDate()
			want := date.YyyyWwD(testcase.sutYear, testcase.sutWeek, date.DayOfWeekSunday)
			calendar_test.AssertEqualDate(t, want, got)
		})
	}
}
...
func TestYearWeek_Compare(t *testing.T) {
	...
	for _, testcase := range testcases {
		sut := date.YearWeekOf(testcase.sutYear, testcase.sutWeek)
		in := date.YearWeekOf(testcase.inYear, testcase.inWeek)

...
func main() {
	...
	for _, sut := range nanos {
		for _, in := range nanos {
			sutSec, sutNano, ok := duration.Decompose(big.NewInt(sut))
			if !ok {
				log.Panicf("%+v", sut)
			}
			inSec, inNano, ok := duration.Decompose(big.NewInt(in))
			if !ok {
				log.Panicf("%+v", in)
			}
			wantSec, wantNano, _ := duration.Decompose((&big.Int{}).Sub(big.NewInt(sut), big.NewInt(in)))

			fmt.Fprintf(out, "%d %d %d %d %d %d\n", sutSec, sutNano, inSec, inNano, wantSec, wantNano)
		}
	}
	log.Println("func (Duration) Sub testcases successfully generated in " + outputPath)
}
